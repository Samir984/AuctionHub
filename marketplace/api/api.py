...
# @api.get("/bids/{auction}/", response=list[BidSchema])
# def list_bids(request, auction: int):
#     bids = Bid.objects.filter(auction=auction)
#     return bids


# @api.post("/bids/", response=BidSchema)
# def create_bid(request, auction: int, bid_amount: float, bid_time: datetime):
#     # Get the auction object
#     auction = get_object_or_404(Auction, id=auction)
